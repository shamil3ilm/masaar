...
| — | **No JWT request had a tenant** | `JwtGuard` establishes the tenant from the token's `org_id` and `role` claims, and `getJWTCustomClaims()` returned an empty array — so the condition was never true and roughly ninety routes ran unscoped. Reads filtered on `org_id = null`; `/api/dashboard` answered 500 to a valid token. `organizations/{id}/switch` set the organization on the request-scoped resolver, so it was forgotten before the response was sent. A sole membership is now claimed at login and `switch` reissues a token carrying the choice. |
| — | **CSR generation never worked** | Three defects, none tested: the subject alternative name declared five `dirName.N` entries where OpenSSL expects one naming a section, so the request-extensions section failed to load; `openssl_pkey_export()` was unchecked and returned an empty private key on failure; and the explicit `$dn` passed to `openssl_csr_new()` meant the config's `organizationIdentifier` — the VAT registration ZATCA requires in the subject — never reached the request. |
| — | **The XAdES signature was over the empty string** | Found while giving `XadesSigner` its first direct test. `sign()` canonicalised `SignedInfo` before attaching it to the document, and `DOMNode::C14N()` returns `""` for a detached subtree without saying so — so every invoice carried an ECDSA signature over nothing and a SignedProperties digest of nothing. Both well-formed, neither verifying. Fixed by attaching before canonicalising; the signature now verifies against the certificate in the document, and reverting the order fails the test. |
| — | **The signature was outside `UBLExtensions`** | `XmlBuilder::addSignatureExtension()` builds the `ExtensionContent` scaffold ZATCA reads and had no callers, so the signer fell back to appending the signature under the invoice root. The scaffold is built now; the invoice hash is unaffected because the document reference strips `UBLExtensions` before digesting. |
| **M-5** | API key hashing | Reopened and closed properly. `8054ba2` added `security.api_key_pepper` and `.env.example` described it as protection against an offline attack on a leaked table — and nothing read it. Secrets were a bare SHA-256. `License::hashSecret()` is now the one place a secret is hashed, HMAC-SHA256 under the pepper. |
...
| **Which invoice type gets the nine-tag QR?** | `DocumentBuilder` gives the cryptographic QR to standard invoices and the five-tag one to simplified, and `QrCodeGenerator`'s own docblocks say the same. That is the opposite of what a simplified invoice appears to need — it is handed to the customer and reported afterwards, with nothing clearing it. The codebase is self-consistent, so this is a doubt rather than a defect, and flipping it on recollection would produce rejected documents either way. **W-5.1**'s fixtures settle it. |
| **ZATCA's cleared invoice is discarded** | `FatooraResponse` parses `clearedInvoice` — the document the authority returns with its own stamp applied — and nothing reads it. For a standard invoice that is the filed artefact, and the taxpayer keeps their own copy instead. Storing it needs a decision on where: beside `signed_xml`, or replacing it. |
