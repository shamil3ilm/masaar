 ...
 * tenant comes from the token, a certificate request is generated by CsrBuilder,
...
class OnboardingFlowTest extends TestCase
{
    ...
    /**
     * The certificate template names ZATCA's environment, and a request whose
     * template does not match the endpoint it reaches is refused. It was fixed
     * to production's in every environment.
     */
    public function test_the_request_names_the_environment(): void
    {
        config(['fatoora.environment' => 'simulation']);
        ...
        Http::assertSent(function ($request) {
            $pem = base64_decode($request['csr'] ?? '');
            $der = base64_decode((string) preg_replace('#-----[^-]+-----|\s#', '', $pem));

            return str_contains($request->url(), '/simulation/')
