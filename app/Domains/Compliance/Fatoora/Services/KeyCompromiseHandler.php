...
class KeyCompromiseHandler
{
    ...
    public function identifyAffectedInvoices(array $incident): array
    {
        ...
        // Collect invoice IDs that have no submission (candidates for offline queue check)
        $invoiceIdsWithoutSubmission = [];
        $invoiceDataMap = [];

        $invoicesQuery->cursor()->each(function ($invoice) use ($submissions, &$invoiceIdsWithoutSubmission, &$invoiceDataMap) {
            $invoiceDataMap[$invoice->id] = [
            ...
            if (!isset($submissions[$invoice->id]) || $submissions[$invoice->id]->isEmpty()) {
                $invoiceIdsWithoutSubmission[] = $invoice->id;
            }
        });

        // Batch-query the offline_queue table for all invoices without submissions
        $invoiceIdsInOfflineQueue = collect();
        if (!empty($invoiceIdsWithoutSubmission)) {
            $invoiceIdsInOfflineQueue = DB::table('offline_queue')
                ->whereIn('invoice_id', $invoiceIdsWithoutSubmission)
                ->pluck('invoice_id')
                ->flip();
        }

        // Categorize invoices using the pre-fetched data
        foreach ($invoiceDataMap as $invoiceId => $invoiceInfo) {
            $submission = ($submissions[$invoiceId] ?? collect())->first();
            ...
            if ($submission) {
                ...
                if ($invoiceIdsInOfflineQueue->has($invoiceId)) {
