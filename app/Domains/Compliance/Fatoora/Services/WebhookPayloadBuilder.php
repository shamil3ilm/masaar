...
class WebhookPayloadBuilder
{
    ...
    public function buildHeaders(array $payload): array
    {
        return [
            ...
            'X-Masaar-Event-ID' => $payload['event_id'],
            'X-Masaar-Event-Type' => $payload['event_type'],
            'X-Masaar-Signature' => $payload['signature'],
            'X-Masaar-Timestamp' => $payload['occurred_at'],
            'X-Masaar-Idempotency-Key' => $payload['idempotency_key'],
...
app.post('/webhooks', (req, res) => {
  ...
  const signature = req.headers['x-masaar-signature'] as string;
...
TS;
     ...
     *         Masaar is not liable for security issues caused by
    ...
    public function getConsumerContract(): array
    {
        return [
            ...
            'liability_notice' => 'Consumers are solely responsible for implementing ' .
                'signature verification and replay protection. Masaar is not liable ' .
