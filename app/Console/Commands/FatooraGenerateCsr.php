...
EOT;
        ...
        // The SDK reports a refused config on stdout and still exits 0 - an
        // invalid csr.invoice.type is announced as an error and the run
        // "succeeds". Trusting the code printed "CSR generated by ZATCA SDK"
        // and then died reading a file that was never written, which reads as
        // a filesystem problem rather than a rejected request.
        if ($returnCode !== 0 || ! is_file($csrOutput) || ! is_file($keyOutput)) {
            throw new \RuntimeException(
                "SDK CSR generation failed (exit {$returnCode}); no CSR was written."
